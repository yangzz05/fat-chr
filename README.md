...
> * **Apple Virtualization Framework**  On Intel, all images should work - see `[tikoci/mikropkl](https://github.com/tikoci/mikropkl)`, which use Apple-based virtualization to run CHR on Intel-based Mac.  On ARM-based M1/M2/M3, CHR using Apple Virtualization has not worked, and likely because not the disk image - RouterOS lack of drivers. _e.g._ Apple uses specific VirtIO drivers that are not present in the RouterOS kernel.
> * OEMU-based images work without re-packaging with QEMU on ARM64.  See **[Pick your CHR download tool**](https://tikoci.github.com/chr-images.html)**, which allow downloading both Apple (always EFI, on Intel, uses the `fat-chr` image) and QEMU (SeamBIOS on X86, EFI on ARM64, always uses MikroTik official CHR packages), including an "Open in UTM" one-click to a router VM, and QEMU help script `qemu.sh` to quickly launch CHR using QEMU at CLI.
